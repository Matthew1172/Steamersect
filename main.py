...
def filter(list_of_games, a):
    ...
    '''
    return [
        k for k, v in build_game_map(list_of_games).items()
        if a(len(list_of_games), v)
    ]
    ...
    return [
        requests.get(
            "https://store.steampowered.com/api/appdetails?appids={}".format(
                app)).json() for app in list_of_games[:j]
    ]
...
def get_game_names(list_of_game_details):
    ...
    '''
    return [
        v['data']['name'] for game in list_of_game_details if game is not None
        for k, v in game.items() if v['success']
    ]
